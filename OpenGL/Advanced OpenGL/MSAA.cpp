...
int main()
{
    ...
    // Framebuffer with MSAA color attachment
    unsigned int framebuffer;
    glGenFramebuffers(1, &framebuffer);
    glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
    
    unsigned int tex4x;
    glGenTextures(1, &tex4x);
    glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, tex4x);
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 4, GL_RGB, SCR_WIDTH * 2, SCR_HEIGHT * 2, GL_TRUE);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, tex4x, 0);
    

    unsigned int rbo;
    glGenRenderbuffers(1, &rbo);
    glBindRenderbuffer(GL_RENDERBUFFER, rbo);
    // Renderbuffer with MSAA
    ...
    // buffer for blit and draw with color attachment only
    unsigned int outputBuffer;
    glGenFramebuffers(1, &outputBuffer);
    glBindFramebuffer(GL_FRAMEBUFFER, outputBuffer);
    
    // non-MSAA 2D texture to draw on screen
