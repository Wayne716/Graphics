...
int main()
{
    ...
    unsigned int flagTexture = loadTexture("texture/christmas.jpg", false);
    bool texInit = false;
    ...
    glm::vec3 center = glm::vec3(0.45, 0.4, 0.48);
    float radius = 0.3f;
    int stacks = 18;
    int sectors = 36;
    
    Shader sph("sphere.vs", "sphere.fs");
    Sphere* sphere = new Sphere(center, radius, stacks, sectors);
    
    float* vertices = &sphere->positions[0];
    unsigned int* indices = &sphere->indices[0];
    ...
    glBufferData(GL_ARRAY_BUFFER, sphere->positions.size() * sizeof(float), vertices, GL_STATIC_DRAW);
    glGenBuffers(1, &sphereEBO);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, sphereEBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sphere->indices.size() * sizeof(unsigned int), indices, GL_STATIC_DRAW);
    glEnableVertexAttribArray(0);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (int i=0; i<steps; ++i)
            cloth->simulate(fps, steps, cp, ext, sphere);
        
        int j = 0;
        if (!texInit) {
            for (int i=0; i<tris; ++i) {
                Triangle* t = cloth->mesh->triangles[i];
                v[j+6] = t->m1->position.x * 2.0f;
                v[j+7] = t->m1->position.z * 2.0f;
                v[j+14] = t->m2->position.x * 2.0f;
                v[j+15] = t->m2->position.z * 2.0f;
                v[j+22] = t->m3->position.x * 2.0f;
                v[j+23] = t->m3->position.z * 2.0f;
                j += 24;
            }
            texInit = true;
        }
        ...
        sph.use();
        sph.setMat4("projection", projection);
        sph.setMat4("view", view);
        sph.setMat4("model", glm::translate(model, center));
